...
export default function AdminOrderDetailPage() {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      // Clear notification badge if it's on
    ...
    return (
      <div className="min-h-screen bg-gray-900 flex items-center justify-center">
        <div className="text-center">
          ...
          <Button onClick={() => navigate(createPageUrl('AdminOrders'))} className="bmw-gradient">
  ...
  return (
    <div className="min-h-screen bg-gray-900 py-8">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <Button variant="ghost" onClick={() => navigate(createPageUrl('AdminOrders'))} className="text-gray-300 hover:text-white mb-6">
          <ArrowLeft className="w-4 h-4 mr-2" />
          Back to All Jobs
        </Button>
        <div className="grid lg:grid-cols-3 gap-8">
          <div className="lg:col-span-1 space-y-6">
            ...
            <StatusManager order={order} onUpdate={handleOrderUpdate} />
          </div>
          <div className="lg:col-span-2">
            <Tabs defaultValue="chat" className="w-full">
              ...
              <TabsContent value="chat" className="mt-4">
                <ChatBox order={order} messages={messages} onMessageSent={loadData} user={user} />
              </TabsContent>
              <TabsContent value="files" className="mt-4">
                <FileInfo order={order} files={files} onFileChange={loadData} user={user} />
              </TabsContent>
            </Tabs>
          </div>
        </div>
