...
export default function FileInfo({ order, files, onFileChange, user }) {
  ...
  const handleFileUpload = async (event) => {
    ...
    try {
      ...
      await FileEntity.create({
        ...
        file_name: file.name,
        file_url: file_url,
        version: (files.filter(f => f.uploaded_by_role === 'admin').length + 1),
      });
  ...
  return (
    <div className="space-y-6">
      <div>
        <h3 className="text-lg font-semibold text-white mb-2">Tune Files (Admin Uploads)</h3>
        <Card className="glass-effect border-gray-700">
          <CardContent className="p-4 space-y-3">
            <ScrollArea className="h-48">
              {adminFiles.length > 0 ? adminFiles.map(file => (
                <div key={file.id} className="flex items-center p-2 bg-gray-800/50 rounded-lg">
                  ...
                  <Button variant="ghost" size="icon" asChild><a href={file.file_url} target="_blank"><Download className="w-4 h-4 text-gray-300" /></a></Button>
                  <Button variant="ghost" size="icon" onClick={() => handleDelete(file.id)}><Trash2 className="w-4 h-4 text-red-500" /></Button>
                </div>
              )) : <p className="text-gray-400 text-sm text-center py-4">No tune files uploaded yet.</p>}
            </ScrollArea>
            <Input type="file" ref={fileInputRef} className="hidden" onChange={handleFileUpload} />
            <Button onClick={handleFileSelect} disabled={isUploading} className="w-full">
              <Upload className="w-4 h-4 mr-2" />
              {isUploading ? "Uploading..." : "Upload New Tune File"}
            </Button>
          </CardContent>
        </Card>
      </div>
          ...
          <CardContent className="p-4">
             <ScrollArea className="h-48">
              {customerFiles.length > 0 ? customerFiles.map(file => (
                <div key={file.id} className="flex items-center p-2 bg-gray-800/50 rounded-lg mb-2">
              ...
              )) : <p className="text-gray-400 text-sm text-center py-4">No files uploaded by customer.</p>}
            </ScrollArea>
          </CardContent>
        </Card>
      </div>
