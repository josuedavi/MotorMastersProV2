  ...
  return (
    <div className="h-full flex flex-col">
            ...
            <div key={msg.id} className={`flex ${msg.sender_role === 'customer' ? 'justify-end' : 'justify-start'}`}>
              <div className={`max-w-xs md:max-w-md p-3 rounded-lg ${msg.sender_role === 'customer' ? 'bg-blue-600 text-white' : 'bg-gray-700 text-gray-200'}`}>
                ...
                {msg.attachments?.map(att => (
                  <a href={att.file_url} key={att.file_url} target="_blank" rel="noreferrer" className="text-xs underline flex items-center mt-1">
      ...
      <div className="mt-4 flex gap-2">
        <Input type="file" ref={fileInputRef} className="hidden" onChange={handleFileUpload} disabled={isSending} />
